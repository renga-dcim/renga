defmodule RengaWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, RengaWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})

    %{user: %{user_fixture() | authenticated_at: Renga.Time.utc_now_ms()}, conn: conn}
  end
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      RengaWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_sudo_mode" do
    ...
    test "redirects when authentication is too old", %{conn: conn, user: user} do
      eleven_minutes_ago = Renga.Time.utc_now_ms() |> DateTime.add(-11, :minute)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        RengaWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
