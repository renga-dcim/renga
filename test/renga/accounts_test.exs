defmodule Renga.AccountsTest do
  ...
  describe "organization memberships" do
    ...
    test "membership uniqueness is scoped by organization and user id", %{
      ...
      assert {:ok, _membership} =
               Accounts.create_organization_membership(organization, %{user_id: user.id})

      assert {:ok, _membership} =
               Accounts.create_organization_membership(other_organization, %{user_id: user.id})
  ...
  describe "generate_user_session_token/1" do
    ...
    test "duplicates the authenticated_at of given user in new token", %{user: user} do
      user = %{user | authenticated_at: DateTime.add(Renga.Time.utc_now_ms(), -3600)}
      token = Accounts.generate_user_session_token(user)
      assert user_token = Repo.get_by(UserToken, token: token)
